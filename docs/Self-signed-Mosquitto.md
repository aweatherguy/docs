...
One note of caution: The `easyrsa` shell script may wind up with the wrong line endings if git is not configured to checkout line endings "as is". When this happens, the shell script will not run in Cygwin, and this problem may be fixed like this:
```
mv easyrsa tmprsa
...
Each Tasmota device needs to be configured with the host name of the server. This is done via the `MQTT_HOST` macro in `user_config_override.h`, and can be overridden in the device's MQTT Configuration web page. The host name string must meet two requirements:
...
- Install BearSSL (see below for help with building BearSSL on Windows machines).
```
...
- MQTT Port -- this often does not come up as 8883 and must be changed
- MQTT Topic
...
One use case is an isolated WiFi network, not connected to the Internet and with no DNS servers. In this case, it may be necessary to assign numeric IP addresses as the Tasmota device's MQTT Host parameter. As mentioned above, the host name (or numeric IP) configured in Tasmota must match the Common Name (CN) in the server's TLS certificate.
...
#### 5.2 File Permissions: Windows

It should be possible to configure permissions so that only the `SYSTEM` user can read the private key certificate file, and when mosquitto is run as a service, it runs under the `SYSTEM` account. Doing this is not trivial and beyond the scope of this tutorial.
...
!!! failure "Repeated step" Repet the following 5.x steps once for every device, changing tasmota_name for each device. Also choose a private key password for each device when prompted.
...
You will now need to convert your certificates to Tasmota commands. 
Credentials are composed of two distinct parts:

- Private Key - this is the secret that will allow your device to prove it is who it pretends to be. Consider this as sensitive as a password. The Private Key is exactly 32 bytes (256 bits).
...
Both of them need to be loaded into flash in each Tasmota device. This is done by entering `TLSKey` commands in each device's web console. Run the following commands and follow the instruction in the console output.
```
...
This can be a challenging task, and a method to accomplish this through Cygwin is described here. It may also help if there are problems running EasyRSA in a native Windows environment.
...
Open a Cygwin terminal window and change to the directory where BearSSL source was unpacked. To access a Windows letter drive such as `F:` in Cygwin, use the path `/cygdrive/f`. 
...
The `brssl.exe` file should be in the `build` directory, and can be copied over to the `easyrsa3` if desired for convenience.
